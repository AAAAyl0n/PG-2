...
void SelectMenuRenderCb_AlignRight::renderCallback(const std::vector<SMOOTH_MENU::Item_t*>& menuItemList,
                                                   ...
                                                   const SMOOTH_MENU::RenderAttribute_t& camera)
{
    ...
    for (int i = 0; i < menuItemList.size(); i++)
    {
        HAL::GetCanvas()->drawString(
            menuItemList[i]->tag.c_str(), menuItemList[i]->x + camera.width - 20, menuItemList[i]->y - camera.y);
    }

    // Render selector
    HAL::GetCanvas()->setColor(TFT_WHITE);
    HAL::GetCanvas()->fillSmoothRoundRectInDifference(selector.x + camera.width - selector.width - 20,
                                                      selector.y - camera.y +
