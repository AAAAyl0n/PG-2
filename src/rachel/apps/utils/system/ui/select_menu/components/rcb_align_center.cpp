...
void SelectMenuRenderCb_AlignCenter::renderCallback(const std::vector<SMOOTH_MENU::Item_t*>& menuItemList,
                                                    ...
                                                    const SMOOTH_MENU::RenderAttribute_t& camera)
{
    ...
    {
        HAL::GetCanvas()->drawString(menuItemList[i]->tag.c_str(), camera.width / 2, menuItemList[i]->y - camera.y);
    }
    ...
    HAL::GetCanvas()->fillSmoothRoundRectInDifference(
        selector.x - camera.x - (selector.width - menuItemList[selector.targetItem]->width) / 2 +
            (camera.width / 2 - menuItemList[selector.targetItem]->width / 2) - 10,
