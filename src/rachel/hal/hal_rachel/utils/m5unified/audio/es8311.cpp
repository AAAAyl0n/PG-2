...
static m5::I2C_Class* _i2c_bus = nullptr;
...
{
    _wire = wire;
    _i2c_bus = nullptr;  // Use TwoWire instead of I2C_Class
}

void es8311_set_i2c_bus(m5::I2C_Class* i2c_bus)
{
    _i2c_bus = i2c_bus;
    _wire = nullptr;     // Use I2C_Class instead of TwoWire
}
...
esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    if (_i2c_bus != nullptr) {
        // Use I2C_Class
        bool result = _i2c_bus->writeRegister8(es->dev_addr, reg_addr, data, 100000);
        return result ? ESP_OK : ESP_FAIL;
    } else if (_wire != nullptr) {
        // Use TwoWire (original implementation)
...
static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    ...
    if (_i2c_bus != nullptr) {
        // Use I2C_Class
        *reg_value = _i2c_bus->readRegister8(es->dev_addr, reg_addr, 100000);
        return ESP_OK;
    } else if (_wire != nullptr) {
        // Use TwoWire (original implementation)
        _wire->beginTransmission(es->dev_addr);
